...
@ti.data_oriented
class Dendrite:
    def __init__(self, dx=0.03, dt=2.e-4, n=512, dtype=ti.f64, n_fold_symmetry=6, angle0=0.):
        self.n = n  # the size of the field
        self.phi = ti.field(dtype=dtype, shape=(n, n))  # phase field indicate solid phase (phi=1) and water vapor phase (phi=0)
        ...
        self.phiRate = ti.Vector.field(4, dtype=dtype, shape=(n, n))  # phiRate[i, j][0] ~  phiRate[i, j][3] 
    ...
    def get_rate(self, rk_loop: int):
        ...
        for i, j in phi:
            ...
            if gradNorm < 1.e-8:
                ...
                epsilons[i, j] = grad_energy_coef * (1. + aniso_magnitude * ti.cos(n_fold_symmetry * (angle - angle0)))
            else:
                angle = ti.atan2(grad[1], grad[0])
                epsilon = grad_energy_coef * (1. + aniso_magnitude * ti.cos(n_fold_symmetry * (angle - angle0)))
        ...
        for i, j in phi:
            ...
            m_chem = self.alpha * ti.atan2(self.gamma * (self.temperature_equi - temperature[i, j]), 1.)
    ...
    def rk4_total_update(self, ):
        ...
        dt, phi, phi_old, temperature, temperature_old, phiRate, temperatureRate = ti.static(
            self.dt, self.phi, self.phi_old, self.temperature, self.temperature_old, self.phiRate, self.temperatureRate)
        for I in ti.grouped(phi):
    ...
    def getDendritic(self, steps=2048):
        ...
        for i in range(steps):
            if i % self.showFrameFrequency == 0:
                ...
                gui_phi.show(
                    path + "time_{:.4f}s.png".format(i * self.dt) 
                        if i % (self.showFrameFrequency * 8) == 0 and self.writeImages 
                        else None)
            self.advance()
